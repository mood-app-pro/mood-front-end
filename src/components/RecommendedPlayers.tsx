      ...
      <div className="container mx-auto px-4 md:px-20 lg:px-40 xl:px-56">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {players.map((player, index) => (
            <div key={index} className="bg-gray-800 rounded overflow-hidden shadow-md flex">
              <div className="w-1/3 flex items-center justify-center">
                <img src={player.image} alt={player.name} className="h-48 w-full object-cover rounded-l" />
              </div>
              <div className="p-4 flex flex-col justify-between w-2/3">
                <div>
                  <h3 className="text-xl font-bold">{player.name}</h3>
                  <p className="text-sm">{player.game}</p>
                  <p className="text-xs mt-2">{player.status}</p>
                </div>
                <div className="flex items-center justify-between mt-4">
                  <div className="flex items-center space-x-2">
                    <span className="text-xs">{player.matches}</span>
                    <span className="text-xs">{player.reviews}</span>
                  </div>
                  <span className="text-sm bg-yellow-500 text-gray-800 px-2 py-1 rounded">{player.rating}</span>
                </div>
              </div>
            </div>
          ))}
